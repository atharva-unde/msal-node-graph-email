...
async function saveTokenToFile(tokenData) {
  try {
    ...
    await fs.writeFile("./msalCache.json", serializedCache);
...
async function loadTokenFromFile(type = "token") {
  try {
    if (type === "token") {
      const data = await fs.readFile(TOKEN_FILE_PATH, "utf8");
      return JSON.parse(data);
    } else if (type === "msalCache") {
      let cacheData = await fs.readFile("./msalCache.json", "utf8");

      // Hack: Deserialize MSAL cache else sometimes msalClient will start acting up
      await msalClient.getTokenCache().deserialize(cacheData);
      const tokenCache = await msalClient.getTokenCache().serialize();
...
async function refreshAccessToken() {
  try {
    let context = await loadTokenFromFile("msalCache");
    const silentRequest = {
    ...
    const newToken = await msalClient.acquireTokenSilent(silentRequest);
...
async function sendEmailViaMicrosoft(emailData) {
  try {
    ...
    if (error.response?.status === 401) {
      throw new Error(
        "Authentication failed. Please re-authorize the application."
      );
    }
...
app.get("/api/token-status", async (req, res) => {
  try {
    ...
    res.json({
      ...
      message: isExpired
        ? "Token expired. Will refresh automatically on next email send."
        : "Token is valid",
    });
  } catch (error) {
...
app.listen(port, async () => {
  ...
  console.log(
    `- GET  /api/mailer/email-servers/microsoft/redirect - OAuth callback`
  );
